	...
	methods: {
		...
		get_components() {
			this.request('get', `${components_url}/${this.device_id}`, this.token, (res) => {
				...
				let c_w = dom.clientWidth;
				// 存一下面板数据 查跳转页类型用
				this.panels = res.data.data.panelParam;
				this.projectid = this.panels[0].projectid;
				...
				this.page_id = this.component_list[0].name;
				this.$nextTick(() => {
		...
		// 根据页面id查询页面类型
		check_page_type(page_id) {
			for (let val of this.panels) {
				if (val.name === page_id) {
					return val.是否是弹窗面板 || false;
				}
			}
		},
	},
