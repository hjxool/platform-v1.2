...
new Vue({
	...
	data: {
		head: {
			...
			calendar_min: new Date(2010, 0, 1),
			start_time: '', //开始日期
			end_time: '', //结束日期
		},
		body: {
	...
	methods: {
		...
		async get_data() {
			...
			// 如果只看我申请的会议
			if (this.head.type !== 'all') {
				body.condition.auditStatus = this.head.type;
			}
			// 如果选择了时间
			...
			for (let val of d.data) {
				let t = {
					...
					place: val.roomName,
					id: val.id,
					start_date: val.startTime.split(' ')[0],
				};
				this.body.list.push(t);
			}
		},
		// 会议标签
		tag_type(status, audit) {
			switch (true) {
				case status === 0 && audit === 2:
					return 'success';
