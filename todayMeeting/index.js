...
let meeting_status = `${url}api-portal/meeting/date/usage`; //
...
new Vue({
  ...
  methods: {
    ...
    getMeetingStatus() {
      this.request(
        "get",
        `${meeting_status}?startTime=${this.calendar.title_year}-${this.calendar.title_month}-01 00:00:00&endTime=${this.calendar.title_year}-${
          this.calendar.title_month
        }-${new Date(this.calendar.title_year, this.calendar.title_month, 0).getDate()} 23:59:59`,
        this.token,
        {},
        (res) => {
          if (res.data.head.success) {
            ...
            console.log(month, this.calendar.title_month);

            // 处理date，保留月份
            const resData = res.data.data.map((item, index) => {
              if (index <= 8) {
                item.date = item.date.substring(9, 10);
              } else {
            ...
            // 过滤有会议的日期
            const filterArr = resData.filter((item) => {
              return item.isUsed;
            });
            const dateStatus = filterArr.map((item) => {
              if (item.date < today && month === this.calendar.title_month) {
                item.bg = "#cecece";
              } else if (item.date > today && month === this.calendar.title_month) {
                item.bg = "#a2ef4d";
              } else if (this.calendar.title_month < month) {
                item.bg = "#cecece";
              } else if (this.calendar.title_month > month) {
                item.bg = "#a2ef4d";
              }
            ...
            this.calendar.days.forEach((item, index) => {
              dateStatus.forEach((val) => {
                if (item.data == val.date) {
                  item.bg = val.bg;
                }
              });
            });
    ...
    // 选择当月月中某天
    select_day(day) {
      this.calendar.select_day = day.data;
      this.get_data(1);
    },
    // 切换会议状态
    ...
    button_show(row_obj, type) {
      switch (type) {
        case "取消":
          if (
            (this.current_user == row_obj.createUser || this.current_user == row_obj.moderatorId) &&
            this.config.cancel_show &&
            row_obj.auditStatus == 2 &&
            row_obj.status === 0
          ) {
            return true;
          } else {
